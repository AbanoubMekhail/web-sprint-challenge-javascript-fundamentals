...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Because of scope function can reach out of there scope into the global scope but it doesnt work the other way around we cant reach a variable iside a function .
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arrry){
    const mappedArr = arrry.map((element)=> {
      return element.animal_name.toLowerCase(); + element.scientific_name.toLowerCase();
        })
    return (mappedArr);
  }
lowerCaseNames(zooAnimals);
  ...
  */
  ...
  function USApop(array){
    const reduceArr = array.reduce((accumulator, currentP)=>{
      return accumulator + currentP.population;
    },0); 
    return reduceArr;
  }
